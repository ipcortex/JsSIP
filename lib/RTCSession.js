...
module.exports = class RTCSession extends EventEmitter
{
  ...
  _iceFailedRecoveryCheck() 
  {
    let gstate = this._connection.iceGatheringState;
    let cstate = this._connection.iceConnectionState;
    ...
    if (gstate === 'complete' && cstate === 'disconnected') 
    {
      ...
      setTimeout(() =>
      {
        gstate = this._connection.iceGatheringState;
        cstate = this._connection.iceConnectionState;

        if (gstate === 'complete' && cstate === 'disconnected') 
        {
          debug('ICE gathering complete but ICE disconnected - restartICE() now');
          this.renegotiate({ rtcOfferConstraints: { restartIce: true } }, () => debug('renegotiate done'));
        }
        else 
        {
  ...
  _createLocalDescription(type, constraints, mangle)
  {
  ...
  _sendReinvite(options = {})
  {
    ...
    this._connectionPromiseQueue = this._connectionPromiseQueue
      .then(() => this._createLocalDescription('offer', rtcOfferConstraints, this._mangleOffer.bind(this)))
      .then((sdp) =>
      {
